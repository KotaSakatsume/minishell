...
/*   Updated: 2025/06/20 14:28:30 by kosakats         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parent_process_cleanup(t_pipeline *pipeline, int prev_pipe[2],
		int pipe_fd[2], pid_t pid, t_shell_env *shell_env)
{
	int	status;
		...
		int prev_pipe[2], int pipe_fd[2])
{
	...
	if (pid == 0)
		...
		parent_process_cleanup(pipeline, prev_pipe, pipe_fd, pid, shell_env);
}
