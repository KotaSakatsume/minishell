...
static void check_and_expand(t_token *token_ptr,t_shell_env *t_shellenv_ptr)
{
	...
	while(serach_start_daller_ptr(token_ptr->value) != NULL)
	{
		...
		if((*(daller_str_ptr + 1) == '?'))
		{
			...
			expandered_str = ft_strdup(ft_itoa(t_shellenv_ptr->exit_status));
			ahead_str = ft_strndup(token_ptr->value,((daller_str_ptr) - (token_ptr->value)));
			behind_str = ft_strndup(daller_end_ptr,ft_strlen(daller_end_ptr));
			temp_str = combine_str_and_free_oldstr(ahead_str,expandered_str);
		...
		else
		{
			...
			indaller_str = ft_strndup((daller_str_ptr + 1),ft_strlen(daller_str_ptr + 1));
